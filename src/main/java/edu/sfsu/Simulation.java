...
public class Simulation {
  ...
  static void prioritySimulation() throws InterruptedException {
    ...
    // Add a few processes
    SimulatedProcess parentProcess = SimulatedProcess.create(0,SimulatedProcess.HIGH_PRIORITY);
    scheduler.addProcess(parentProcess);
    sleep(2000);

    SimulatedProcess firstChild = SimulatedProcess.create(parentProcess.processNumber(),SimulatedProcess.NORMAL_PRIORITY);
    scheduler.addProcess(firstChild);
    sleep(2000);

    SimulatedProcess secondChild = SimulatedProcess.create(parentProcess.processNumber(),SimulatedProcess.LOW_PRIORITY);
    scheduler.addProcess(secondChild);
    sleep(2000);

    SimulatedProcess thirdChild = SimulatedProcess.create(parentProcess.processNumber(),SimulatedProcess.LOW_PRIORITY);
